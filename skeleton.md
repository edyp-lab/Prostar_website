...
* Prostar version 1.30.7 has been released on the Bioconductor (release 3.16) and is deployed as a [Zero-install](#zero-install)  zip.
...
Download <a href="http://prabig-prostar.univ-lyon1.fr/ProstarZeroInstall/Prostar_1.30.7.zip" onclick="ga('event', 'send', { 'event_category': 'file_download',  'event_name': 'Download Prostar_1.30.c7.zip'});">**Prostar 1.30.7 zip file**</a>
...
```
docker pull ghcr.io/prostarproteomics/prostar:1.30.7

```

There are two ways to launch Prostar's container. Using command line, type:
```
docker run -it -p 3838:3838 ghcr.io/prostarproteomics/prostar:1.30.7
```

Then, open your default web browser onto the following URL: http://localhost:3838.

If an executable file is preferred, then create the launch script referred to as 'docker_launch_prostar.sh' with the following
lines:
